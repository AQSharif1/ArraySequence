package com.ProgB;
// File: IntArraySeq2b.java 

/******************************************************************************
 * A IntArraySeq2b is a collection of int numbers. The sequence can have a
 ...
 *
 * @see <A HREF="../../../../edu/colorado/collections/IntArraySeq2b.java"> Java
 *      Source Code for this class
 *      (www.cs.colorado.edu/~main/edu/colorado/collections/IntArraySeq2b.java)
 *      </A>
 *
 *@author Abdul
 ******************************************************************************/
public class IntArraySeq2b {
	// Invariant of the IntArraySeq2b class:
	 ...
	 * @exception OutOfMemoryError Indicates insufficient memory for: new int[10].
	 **/
	public IntArraySeq2b() {
		try {
			// Set a default capacity for new IntArraySeq2b's.
			int INITAL_CAPACITY = 10;

			// Set each instance variable to its initial value.
			data = new int[INITAL_CAPACITY];
			manyItems = 0;
			currentIndex = 0;
		} // end try
	...
	public IntArraySeq2b(IntArraySeq2b intArray) {
		this.currentIndex = intArray.currentIndex;
		this.data = intArray.data;
		this.manyItems = intArray.manyItems;
	}
	 ...
	 *                                     int[initialCapacity].
	 **/
	public IntArraySeq2b(int initialCapacity) {
		try {
			// Set each instance variable to its initial value.
	...
	}// end IntArraySeq2b(int initialCapacity) method
	...
	public int setCurrent(int n) {
		// 'n' must range from 1 to manyItems and the IntArraySeq2b may not be empty.
	...
	public int getElement(int element) {
		...
		if (i == manyItems)
			...
			return 0;
	}// end getElement(int element) method
	...
	public void ensureCapacity(int minimumCapacity) {
		try {
			if (getCapacity() < minimumCapacity) {
				...
				int[] expandData = new int[minimumCapacity];
			...
			throw new OutOfMemoryError(
					"This sequence capacity is too large! There is not enough memory to store a sequence of capacity "
							+ minimumCapacity + "! "
							+ "Note: The add methods int the sequence capacity if maximum capacity has already been reached. "
	...
	public void trimToSize() {
		try {
			if (data.length > manyItems) {
				// Create a new int[] of size manyItems that data will point to after running
				// this method.
	...
	public void addAfter(int element) {
		...
		if (isCurrent()) {
			// Move all elements up an index, beginning at the end of the IntArraySeq2b and
	...
	public void addBefore(int element) {
		...
		else { // currentIndex is beyond the last element or the IntArraySeq2b is empty.
	 ...
	 **/
	public void addAll(IntArraySeq2b addend) {
		// Make sure there is enough capacity to add the other IntArraySeq2b.
		ensureCapacity(manyItems + addend.manyItems);

		// Copy the addend sequence to the end of the invoked sequence.
		System.arraycopy(addend.data, 0, data, manyItems, addend.manyItems);
		manyItems += addend.manyItems;
	}// end addAll(IntArraySeq2b addend) method
	 ...
	 *       sequence to fail.
	 **/
	public static IntArraySeq2b concatenation(IntArraySeq2b s1, IntArraySeq2b s2) {
		try {
			// Create a new IntArraySeq2b large enough to store the s1 and s2
			// IntArraySeq2b's.
			IntArraySeq2b newSequence = new IntArraySeq2b(s1.manyItems + s2.manyItems);

			// Copy IntArraySeq2b s1 and s2 to the new IntArraySeq2b, newSequence.
	...
	}// end concatenation(IntArraySeq2b s1, IntArraySeq2b s2) method
	 ...
	 *//**
		 ...
		 *                             clone.
		 **//*
			 * public IntArraySeq2b clone() { //Create a new IntArraySeq2b that will be
			 * returned as the clone of the invoked IntArraySeq2b. IntArraySeq2b answer;
			 * 
			 * try{ //Clone the instance variables of the invoked IntArraySeq2b and assign
			 * them to the answer IntArraySeq2b. answer = (IntArraySeq2b) super.clone( );
			 * }//end try catch (CloneNotSupportedException e){ // This exception should not
			 * occur. But if it does, it would probably indicate a programming error that
			 * made super.clone unavailable. // The most common error would be forgetting
			 * the "Implements Cloneable" clause at the start of this class. throw new
			 * RuntimeException ("This class does not implement Cloneable"); }//end catch
			 * catch (OutOfMemoryError e){ throw new OutOfMemoryError
			 * ("There is not enough memory available to clone this sequence!"); }//end
			 * catch //Copy the information in the data instance variable (int[]) from the
			 * invoked IntArraySeq2b to its clone. answer.data = data.clone( ); return
			 * answer; }//end IntArraySeq2b clone() method
			 */


	/**
	 * A method to compare two IntArraySeq2b objects and determine if they are
	 * equivalent.
	 * 
	...
	public boolean equals(Object obj) {
		...
		// Verify 1) That obj is a IntArraySeq2b.
		if (obj instanceof IntArraySeq2b) {
			IntArraySeq2b candidate = (IntArraySeq2b) obj;
			// Verify 2) That candidate has the same number of elements as the invoked
			// IntArraySeq2b.
			if (this.manyItems == candidate.manyItems) {
				// Verify 3) That the elements in candidate and the invoked IntArraySeq2b are
	...
	public String toString() {
		...
		if (manyItems > 0) {
			for (int i = 0; i < manyItems; i++)
				dataString = dataString.append(data[i] + " ");
		} // end if
		else
